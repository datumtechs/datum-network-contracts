...
contract('DataTokenFactory', (accounts) => {
  ...
  beforeEach(async () => {
    tokenTemplate = await DataTokenTemplate.new();
    factory = await DataTokenFactory.new(tokenTemplate.address);
    const res = await factory.createToken(tokenName, tokenSymbol, cap, initialSupply, tokenProof);
    const tokenCreatedLog = res.logs.find(
      element => element.event.match('TokenCreated') &&
        element.address.match(factory.address)
    );
    token = await DataTokenTemplate.at(tokenCreatedLog.args.newTokenAddress);
  })

  it('events: should fire TokenCreated and TokenRegistered event successfully', async () => {
    const res = await factory.createToken(tokenName, tokenSymbol, cap, initialSupply, tokenProof);
    ...
    assert.strictEqual(tokenCreatedLog.args.templateAddress, tokenTemplate.address);

    assert.strictEqual(tokenCreatedLog.args.tokenName.toString(), Web3.utils.sha3(tokenName));

    const tokenRegisteredLog = res.logs.find(
        element => element.event.match('TokenRegistered') &&
          element.address.match(factory.address)
      );

    assert.strictEqual(tokenRegisteredLog.args.tokenAddress, tokenCreatedLog.args.newTokenAddress);

    assert.strictEqual(tokenRegisteredLog.args.tokenName.toString(), tokenName);

    assert.strictEqual(tokenRegisteredLog.args.tokenSymbol.toString(), tokenSymbol);

    assert.strictEqual(tokenRegisteredLog.args.tokenCap.toString(), '1000000');

    assert.strictEqual(tokenRegisteredLog.args.initialSupply.toString(), '10000');

    assert.strictEqual(tokenRegisteredLog.args.registeredBy, accounts[0]);

    assert.strictEqual(tokenRegisteredLog.args.proof.toString(), tokenProof);
