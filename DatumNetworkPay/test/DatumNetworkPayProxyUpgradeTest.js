...
contract('DatumNetworkPayProxyUpgrade', (accounts) => {
  ...
  beforeEach(async () => {
    ...
    datumNetworkPayImplement = await erc1967.getImplementationAddress(datumNetworkPay.address);
    console.log("DatumNetworkPay implement:", datumNetworkPayImplement);

    datumNetworkPay = await upgradeProxy(datumNetworkPay.address, DatumNetworkPayV2);
    console.log('upgrade DatumNetworkPay:', datumNetworkPay.address);

    datumNetworkPayImplement = await erc1967.getImplementationAddress(datumNetworkPay.address);
    console.log("upgrade DatumNetworkPay implement:", datumNetworkPayImplement);
  ...
  it('events: should fire PrepayEvent and SettleEvent event successfully', async () => {
    await datumNetworkPay.newAuthorize(accounts[1]);
    var addressArray = await datumNetworkPay.whitelist.call(accounts[0]);
